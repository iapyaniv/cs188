...
def fillObsCPT(bayesNet, gameState):
    ...
    NOTES:
    It's easiest to first loop over possible house positions, then over possible
    walls for each house, and finally over assignments to (wall color, ghost 
    house position, food house position) triples. Remember to create a separate
    factor for every one of the 4*7=28 possible observation positions.

    Use gameState.getHouseWalls(housePos). The possible house wall locations are
    the same as the observation locations (there are only 2 houses and 4 possible
    house locations), but we have observations surrounding each of the possible
    house centers). The house centers are different than the house wall locations
    (walls are 7 squares adjacent to the center). 

    getPossibleHouses and getHouseWalls will both return a list of (x, y) tuples
    with coordinates. However, the variable FOOD_HOUSE_VAR has values which are
    strings ("topLeft", "topRight", ...). 
    """

    def getAdjacentHouseVal(gameState, obsPos):
        """
        Helper function for fillObsCPT, returns value of adjacent house center
        (one of four possible fixed positions representing corners of game map).
        """
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                wallColor = assignment[obsVar]
                ghostHousePosition = assignment[GHOST_HOUSE_VAR]
                foodHousePosition = assignment[FOOD_HOUSE_VAR]
                adjacentHousePosition = getAdjacentHouseVal(gameState, obsPos)

                if adjacentHousePosition == foodHousePosition:
                    if wallColor == RED_OBS_VAL:
                        obsFactor.setProbability(assignment, PROB_FOOD_RED)
                    elif wallColor == BLUE_OBS_VAL:
                        obsFactor.setProbability(assignment, 1 - PROB_FOOD_RED)
                    else: #if wallColor == NO_OBS_VAL
                        obsFactor.setProbability(assignment, 0)

                elif adjacentHousePosition == ghostHousePosition:
                    if wallColor == RED_OBS_VAL:
                    ...
                    if wallColor == RED_OBS_VAL or wallColor == BLUE_OBS_VAL:
                        obsFactor.setProbability(assignment, 0)
                    else: #if wallColor == NO_OBS_VAL
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        foodLeftAssignmentDict = {FOOD_HOUSE_VAR: TOP_LEFT_VAL, GHOST_HOUSE_VAR: TOP_RIGHT_VAL}
        foodRightAssignmentDict = {FOOD_HOUSE_VAR: TOP_RIGHT_VAL, GHOST_HOUSE_VAR: TOP_LEFT_VAL}

        for variable, observation in evidence.iteritems():
            foodLeftAssignmentDict[variable] = observation
            foodRightAssignmentDict[variable] = observation

        leftProbability = factor.getProbability(foodLeftAssignmentDict)
        rightProbability = factor.getProbability(foodRightAssignmentDict)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        NOTES: First compute p(foodHouse = topLeft and ghostHouse = topRight | evidence)
        and p(foodHouse = topRight and ghostHouse = topLeft | evidence)
        """

        expectedValue = 0 
        for probability, observation in self.getExplorationProbsAndOutcomes(evidence):
            leftExpectedValue, rightExpectedValue = self.computeEnterValues(observation, enterEliminationOrder)
            expectedValue += probability * max(leftExpectedValue, rightExpectedValue)
